...
";
        ...
        public void AddSource()
        {
            ...
            foreach (var path in this.all_source_files)
            {
                ...
                f = (
                        f.StartsWith(
                            CGen.TargetName((TargetType)this.config.target) + '/')
                        ? f.Substring((CGen.TargetName((TargetType)this.config.target) + '/').Length)
                        : f
                        );
                ...
                if (config.grammar_name + ".g4" == f &&
                    !(f == this.parser_src_grammar_file_name || f == this.lexer_src_grammar_file_name))
                {
                    continue;
                }
                string to = null;
