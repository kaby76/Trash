...
namespace org.apache.xml.utils
{
	...
	public class URI
	{
		internal const long serialVersionUID = 7096266377907081897L;
...
//ORIGINAL LINE: public URI(String p_scheme, String p_userinfo, String p_host, int p_port, String p_path, String p_queryString, String p_fragment) throws MalformedURIException
	  public URI(string p_scheme, string p_userinfo, string p_host, int p_port, string p_path, string p_queryString, string p_fragment)
	  {
		...
		if (!string.ReferenceEquals(p_path, null))
		{
		  ...
		  {
			throw new MalformedURIException(XMLMessages.createXMLMessage(XMLErrorResources.ER_NO_QUERY_STRING_IN_PATH, null)); //"Query string cannot be specified in path and query string!");
		  }

		  if (p_path.IndexOf('#') != -1 && !string.ReferenceEquals(p_fragment, null))
		  {
			throw new MalformedURIException(XMLMessages.createXMLMessage(XMLErrorResources.ER_NO_FRAGMENT_STRING_IN_PATH, null)); //"Fragment cannot be specified in both the path and fragment!");
		  }
		}
...
//ORIGINAL LINE: private void initialize(URI p_base, String p_uriSpec) throws MalformedURIException
	  private void initialize(URI p_base, string p_uriSpec)
	  {
		...
		{
		  throw new MalformedURIException(XMLMessages.createXMLMessage(XMLErrorResources.ER_CANNOT_INIT_URI_EMPTY_PARMS, null)); //"Cannot initialize URI with empty parameters.");
		}
		...
		else
		{
		  ...
		  if (!string.ReferenceEquals(m_scheme, null) && p_base != null)
		  {
			...
			//
			if (uriSpec.StartsWith("/", StringComparison.Ordinal) || !m_scheme.Equals(p_base.m_scheme) || !p_base.SchemeSpecificPart.StartsWith("/", StringComparison.Ordinal))
			{
			  p_base = null;
			}
		  }
...
//ORIGINAL LINE: private void initializePath(String p_uriSpec) throws MalformedURIException
	  private void initializePath(string p_uriSpec)
	  {
		...
		while (index < end)
		{
		  ...
		  if (testChar == '%')
		  {
			...
			{
			  throw new MalformedURIException(XMLMessages.createXMLMessage(XMLErrorResources.ER_PATH_CONTAINS_INVALID_ESCAPE_SEQUENCE, null)); //"Path contains invalid escape sequence!");
			}
		  }
		  else if (!isReservedCharacter(testChar) && !isUnreservedCharacter(testChar))
		  {
			if ('\\' != testChar)
			{
			  throw new MalformedURIException(XMLMessages.createXMLMessage(XMLErrorResources.ER_PATH_INVALID_CHAR, new object[]{testChar.ToString()})); //"Path contains invalid character: "
			}
											  //+ testChar);
		  }
	  ...
	  public virtual string SchemeSpecificPart
	  {
		  get
		  {
	  ...
	  public virtual string getPath(bool p_includeQueryString, bool p_includeFragment)
	  {
...
//ORIGINAL LINE: public void appendPath(String p_addToPath) throws MalformedURIException
	  ...
	  ///         otherwise </returns>
	  public virtual bool GenericURI
	  {
		  get
		  {
