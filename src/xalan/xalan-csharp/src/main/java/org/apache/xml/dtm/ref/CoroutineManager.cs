...
namespace org.apache.xml.dtm.@ref
{
	...
	/// <summary>
	/// <para>Support the coroutine design pattern.</para>
	/// 
	/// <para>A coroutine set is a very simple cooperative non-preemptive
	...
	/// </ul>
	/// 
	/// <para>Coroutines can be thought of as falling somewhere between pipes and
	...
	/// control to the other. </para>
	/// 
	/// <para>One classic application of coroutines is in compilers, where both
	...
	/// more than two coroutines are involved.</para>
	/// 
	/// <para>Most coroutine systems also provide a way to pass data between the
	...
	/// directly shared between them with only minimal precautions.</para>
	/// 
	/// <para>"Note: This should not be taken to mean that producer/consumer
	...
	/// coroutines for a clear solution.</para>
	/// 
	/// <para>The fact that only one of a group of coroutines is running at a
	...
	/// simple function call.</para>
	/// 
	/// <para>This version is built on top of standard Java threading, since
	/// that's all we have available right now. It's been encapsulated for
	/// code clarity and possible future optimization.</para>
	/// 
	/// <para>(Two possible approaches: wait-notify based and queue-based. Some
	...
	/// concern.)</para>
	/// 
	/// <para>%TBD% THIS SHOULD BE AN INTERFACE, to facilitate building other
	...
	/// can be built on top of this.</para>
	/// 
	/// </summary>
	public class CoroutineManager
	{
	  ...
	  /// 
	  /// </summary>
	  internal BitArray m_activeIDs = new BitArray();
	  ...
	  /// @xsl.usage internal
	  /// </summary>
	  internal const int m_unreasonableId = 1024;
	  ...
	  /// 
	  /// </summary>
	  internal object m_yield = null;

	  // Expose???
	  internal const int NOBODY = -1;
	  internal const int ANYBODY = -1;
	  ...
	  /// group.
	  /// </summary>
	  internal int m_nextCoroutine = NOBODY;

	  /// <summary>
	  /// <para>Each coroutine in the set managed by a single
	  ...
	  /// approaches.</para>
	  /// 
	  /// <para>%REVIEW% We could use an object as the identifier. Not sure
	  /// it's a net gain, though it would allow the thread to be its own
	  /// ID. Ponder.</para>
	  /// </summary>
	  /// <param name="coroutineID">  If >=0, requests that we reserve this number.
	  /// If <0, requests that we find, reserve, and return an available ID
	  /// number.
	  /// </param>
	  /// <returns> If >=0, the ID number to be used by this coroutine. If <0,
	  ...
	  ///  </returns>
	  public virtual int co_joinCoroutineSet(int coroutineID)
	  {
		  lock (this)
		  {
			if (coroutineID >= 0)
			...
			m_activeIDs.Set(coroutineID, true);
			return coroutineID;
		  }
	  }
	  ...
	  /// </summary>
	  /// <param name="thisCoroutine"> the identifier of this coroutine, so we can
	  /// recognize when we are being resumed. </param>
	  /// <exception cref="java.lang.NoSuchMethodException"> if thisCoroutine isn't
...
//ORIGINAL LINE: public synchronized Object co_entry_pause(int thisCoroutine) throws java.lang.NoSuchMethodException
	  ...
	  /// </summary>
	  /// <param name="arg_object"> A value to be passed to the other coroutine. </param>
	  /// <param name="thisCoroutine"> Integer identifier for this coroutine. This is the
	  /// ID we watch for to see if we're the ones being resumed. </param>
	  /// <param name="toCoroutine">  Integer identifier for the coroutine we wish to
	  /// invoke. </param>
...
//ORIGINAL LINE: public synchronized Object co_resume(Object arg_object,int thisCoroutine,int toCoroutine) throws java.lang.NoSuchMethodException
	  public virtual object co_resume(object arg_object, int thisCoroutine, int toCoroutine)
	  {
		  lock (this)
		  {
			...
			{
			  throw new java.lang.NoSuchMethodException(XMLMessages.createXMLMessage(XMLErrorResources.ER_COROUTINE_NOT_AVAIL, new object[]{Convert.ToString(toCoroutine)})); //"Coroutine not available, id="+toCoroutine);
			}
	  ...
	  /// </summary>
	  /// <param name="thisCoroutine"> Integer identifier for the coroutine requesting exit.
	  ///  </param>
	  public virtual void co_exit(int thisCoroutine)
	  {
		  lock (this)
		  {
			m_activeIDs.Set(thisCoroutine, false);
	  ...
	  /// </summary>
	  /// <param name="arg_object">    A value to be passed to the other coroutine. </param>
	  /// <param name="thisCoroutine"> Integer identifier for the coroutine leaving the set. </param>
	  /// <param name="toCoroutine">   Integer identifier for the coroutine we wish to
	  /// invoke. </param>
	  /// <exception cref="java.lang.NoSuchMethodException"> if toCoroutine isn't a
	  /// registered member of this group. %REVIEW% whether this is the best choice.
