...
namespace org.apache.xalan.xsltc.util
{
	...
	/// </summary>
	public sealed class IntegerArray : ICloneable
	{
		...
		public IntegerArray() : this(InitialSize)
		{
		}
		...
		public IntegerArray(int[] array) : this(array.Length)
		{
		Array.Copy(array, 0, _array, 0, _free = _size);
		}
...
//ORIGINAL LINE: final IntegerArray clone = new IntegerArray(_free > 0 ? _free : 1);
		IntegerArray clone = new IntegerArray(_free > 0 ? _free : 1);
		Array.Copy(_array, 0, clone._array, 0, _free);
		clone._free = _free;
		return clone;
		}
		...
		{
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
//ORIGINAL LINE: final int[] result = new int[cardinality()];
		int[] result = new int[cardinality()];
		Array.Copy(_array, 0, result, 0, cardinality());
		return result;
		}
...
//ORIGINAL LINE: final int newSize = _free + other._free;
	...
	// System.out.println("IntegerArray.merge() end");
		...
		{
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
//ORIGINAL LINE: final int q = partition(array, p, r);
		...
		{
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
//ORIGINAL LINE: final int x = array[(p + r) >>> 1];
		int x = array[(int)((uint)(p + r) >> 1)];
		int i = p - 1;
		int j = r + 1;
		...
		{
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
//ORIGINAL LINE: final int[] newArray = new int[_size = size];
		int[] newArray = new int[_size = size];
		Array.Copy(_array, 0, newArray, 0, _free);
		_array = newArray;
		}
