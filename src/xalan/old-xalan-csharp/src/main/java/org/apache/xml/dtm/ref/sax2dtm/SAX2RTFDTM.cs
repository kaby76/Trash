...
namespace org.apache.xml.dtm.@ref.sax2dtm
{
	...
	/// 
	/// </summary>
	public class SAX2RTFDTM : SAX2DTM
	{
	  /// <summary>
	  ...
	  internal IntStack mark_size = new IntStack();
	  /// <summary>
	  /// Tail-pruning mark: Number of data items in use </summary>
	  internal IntStack mark_data_size = new IntStack();
	  /// <summary>
	  /// Tail-pruning mark: Number of size-of-data fields in use </summary>
	  internal IntStack mark_char_size = new IntStack();
	  /// <summary>
	  /// Tail-pruning mark: Number of dataOrQName slots in use </summary>
	  internal IntStack mark_doq_size = new IntStack();
	  /// <summary>
	  ...
	  /// 
	  /// </summary>
	  internal IntStack mark_nsdeclset_size = new IntStack();
	  /// <summary>
	  ...
	  /// some storage.
	  /// </summary>
	  internal IntStack mark_nsdeclelem_size = new IntStack();

	  /// <summary>
	  /// Tail-pruning mark:  initial number of nodes in use
	  /// </summary>
	  internal int m_emptyNodeCount;

	  /// <summary>
	  /// Tail-pruning mark:  initial number of namespace declaration sets
	  /// </summary>
	  internal int m_emptyNSDeclSetCount;

	  /// <summary>
	  /// Tail-pruning mark:  initial number of namespace declaration elements
	  /// </summary>
	  internal int m_emptyNSDeclSetElemsCount;

	  /// <summary>
	  /// Tail-pruning mark:  initial number of data items in use
	  /// </summary>
	  internal int m_emptyDataCount;

	  /// <summary>
	  /// Tail-pruning mark:  initial number of characters in use
	  /// </summary>
	  internal int m_emptyCharsCount;

	  /// <summary>
	  /// Tail-pruning mark:  default initial number of dataOrQName slots in use
	  /// </summary>
	  internal int m_emptyDataQNCount;

	  public SAX2RTFDTM(DTMManager mgr, Source source, int dtmIdentity, DTMWSFilter whiteSpaceFilter, XMLStringFactory xstringfactory, bool doIndexing) : base(mgr, source, dtmIdentity, whiteSpaceFilter, xstringfactory, doIndexing)
	  {
		...
		m_emptyNSDeclSetCount = (m_namespaceDeclSets == null) ? 0 : m_namespaceDeclSets.Count;
	  ...
	  /// tail-pruned.
	  /// </summary>
	  ///  <returns> int Node handle of Document node, or null if this DTM does not
	  ...
	  /// </summary>
	  /// <param name="nodeIdentifier"> the id of the starting node. </param>
...
//ORIGINAL LINE: public void endDocument() throws org.xml.sax.SAXException
	  ...
	  /// 
	  /// </summary>
	  public virtual void pushRewindMark()
	  {
		if (m_indexing || m_elemIndexes != null)
		{
		  throw new System.NullReferenceException("Coding error; Don't try to mark/rewind an indexed DTM");
		}

		// Values from DTMDefaultBase
		// %REVIEW% Can the namespace stack sizes ever differ? If not, save space!
		mark_size.push(m_size);
		mark_nsdeclset_size.push((m_namespaceDeclSets == null) ? 0 : m_namespaceDeclSets.Count);
	  ...
	  /// </summary>
	  /// <returns> true if and only if the pop completely emptied the
	  /// RTF. That response is used when determining how to unspool
	  /// RTF-started-while-RTF-open situations.
	  ///  </returns>
	  public virtual bool popRewindMark()
	  {
		bool top = mark_size.empty();

		m_size = top ? m_emptyNodeCount : mark_size.pop();
		m_exptype.Size = m_size;
		m_firstch.Size = m_size;
		m_nextsib.Size = m_size;
		m_prevsib.Size = m_size;
		m_parent.Size = m_size;
		...
		{
		  m_namespaceDeclSets.Capacity = ds;
		}

		int ds1 = top ? m_emptyNSDeclSetElemsCount : mark_nsdeclelem_size.pop();
		if (m_namespaceDeclSetElements != null)
		{
		  m_namespaceDeclSetElements.Size = ds1;
		}

		// Values from SAX2DTM - m_data always has a reserved entry
		m_data.Size = top ? m_emptyDataCount : mark_data_size.pop();
		m_chars.Length = top ? m_emptyCharsCount : mark_char_size.pop();
		m_dataOrQName.Size = top ? m_emptyDataQNCount : mark_doq_size.pop();
	  ...
	  ///  </returns>
	  public virtual bool TreeIncomplete
	  {
		  get
		  {
