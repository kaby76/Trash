...
namespace org.apache.xml.dtm
{
	...
	/// function.
	/// 
	/// <para>A DTMAxisTraverser can probably not traverse a reverse axis in
	/// document order.</para>
	/// 
	...
	public abstract class DTMAxisTraverser
	{
	  ...
	  public virtual int first(int context)
	  {
	  ...
	  /// </param>
	  /// <returns> the first node in the traversal. </returns>
	  public virtual int first(int context, int extendedTypeID)
	  {
	  ...
	  /// <param name="context"> The context node of this traversal. This is the point
	  /// of origin for the traversal -- its "root node" or starting point. </param>
	  /// <param name="current"> The current node of the traversal. This is the last known
	  ...
	  /// previous traversal step. For the first traversal step, context
	  /// should be set equal to current. Note that in order to test whether
	  /// context is in the set, you must use the first() method instead. </param>
	  /// <param name="extendedTypeID"> The extended type ID that must match.
	  /// </param>
	  /// <returns> the next node in the iteration, or DTM.NULL. </returns>
