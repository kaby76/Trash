namespace LanguageServer
{
    ...
    public class ConvertPest
    {
        ...
        public Dictionary<string, string> Try(string ffn, string input, string out_type = "antlr4")
        {
            ...
            PestLexer lexer = new PestLexer(str);
            CommonTokenStream tokens = new CommonTokenStream(lexer);
            PestParser parser = new PestParser(tokens);
            var elistener = new ErrorListener<IToken>(parser, lexer, 0);
            parser.AddErrorListener(elistener);
            var tree = parser.grammar_rules();
            string error_file_name = ffn.Replace(".g4", ".txt");
            if (elistener.had_error)
            {
                results.Add(ffn.Replace(".pest", ".txt"), errors.ToString());
                return results;
            }
            else
            {
  ...
  //          ParseTreeWalker.Default.Walk(listener, tree);
        ...
        bigexit:
            ...
            results.Add(ffn.Replace(".pest", ".g4"), sb.ToString());
            results.Add(ffn.Replace(".pest", ".txt"), errors.ToString());
            return results;
        }
    }
