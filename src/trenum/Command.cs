namespace Trash
{
    ...
    class Command
    {
        public string Help()
        {
            using (Stream stream = this.GetType().Assembly.GetManifestResourceStream("trenum.readme.md"))
                ...
                if (expr != null)
                {
                    ...
                    using (ParseTreeEditing.AntlrDOM.AntlrDynamicContext dynamicContext = ate.Try(root, parser))
                    {
                        nodes = engine.parseExpression(expr,
                            ...
                            .Select(x => (x.NativeValue as ParseTreeEditing.AntlrDOM.AntlrElement).AntlrIParseTree as TerminalNodeImpl).ToList();
                    }
                }
