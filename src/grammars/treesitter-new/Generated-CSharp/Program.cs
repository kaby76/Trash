...
public class Program
{
    ...
    public static string StartSymbol { get; set; } = "json5";
    public static string Input { get; set; }
    public static IParseTree Parse(string input)
    {
        var str = new AntlrInputStream(input);
        var lexer = new JSON5Lexer(str);
        Lexer = lexer;
        var tokens = new CommonTokenStream(lexer);
        TokenStream = tokens;
        var parser = new JSON5Parser(tokens);
        Parser = parser;
    ...
    static void Main(string[] args)
    {
        ...
        // Convert tree sitter grammar to normal EBNF because tree shitter is really stupid.
        Class1.MyMain(tree, parser);
