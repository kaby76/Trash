...
files2=`dotnet trash glob '../examples/*.ixml' '../examples/ixml/samples/**/*.ixml' '!../examples/ixml/samples/**/*.decorated.ixml' | grep -v '[.]errors$' | grep -v '[.]tree$' | grep -v '[.]trq$'`
files=()
for f in $files2
do
...
# Group parsing.
echo "${files[*]}" | dotnet trash wdog dotnet trash perf -x -c ar | grep -v '^0' | awk '{print $2}' | sort -u
status=$?
