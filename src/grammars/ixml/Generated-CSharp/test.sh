...
files2=`dotnet trash glob '../examples/*.ixml' '../examples/ixml/samples/**/*.ixml' '!../examples/ixml/samples/**/*.decorated.ixml' | tr -d '\r' | grep -v '[.]errors$' | grep -v '[.]tree$' | grep -v '[.]trq$'`
files=()
for f in $files2
do
...
assertions_err=0
for file in `dotnet trash glob '../examples/*.ixml' '../examples/ixml/samples/**/*.ixml' '!../examples/ixml/samples/**/*.decorated.ixml' | grep -v '[.]errors$' | grep -v '[.]tree$' | grep -v '[.]trq$'`
do
    trq=$file.trq
    if [ -f "$trq" ]
    then
