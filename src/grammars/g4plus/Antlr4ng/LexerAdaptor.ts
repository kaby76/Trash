...
export abstract class LexerAdaptor extends Lexer {
    ...
    public override emit(): Token {
        if ((this.type === G4PlusLexer.OPTIONS || this.type === G4PlusLexer.TOKENS
            || this.type === G4PlusLexer.CHANNELS)
            && this.currentRuleType === Token.INVALID_TYPE) {
            // Enter prequel construct ending with an RBRACE.
            this.currentRuleType = LexerAdaptor.PREQUEL_CONSTRUCT;
        } else if (this.type === G4PlusLexer.OPTIONS && this.currentRuleType === G4PlusLexer.TOKEN_REF) {
            this.currentRuleType = LexerAdaptor.OPTIONS_CONSTRUCT;
        } else if (this.type === G4PlusLexer.RBRACE
            && this.currentRuleType === LexerAdaptor.PREQUEL_CONSTRUCT) {
        ...
        } else if (this.type === G4PlusLexer.AT && this.currentRuleType === Token.INVALID_TYPE) {
            // Enter action.
            this.currentRuleType = G4PlusLexer.AT;
        } else if (this.type === G4PlusLexer.SEMI
            && this.currentRuleType === LexerAdaptor.OPTIONS_CONSTRUCT) {
            // ';' in options { .... }. Don't change anything.
        } else if (this.type === G4PlusLexer.ACTION && this.currentRuleType === G4PlusLexer.AT) {
            // Exit action.
            this.currentRuleType = Token.INVALID_TYPE;
