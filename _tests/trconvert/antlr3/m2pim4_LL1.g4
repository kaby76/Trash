...
fragment
LETTER :
	'A' .. 'Z' | 'a' .. 'z' // make ANTLRworks display separate branches
	;
...
fragment
DIGIT :
	OCTAL_DIGIT | '8' | '9' // make ANTLRworks display separate branches
	;
...
fragment
HEX_DIGIT :
	DIGIT | 'A' | 'B' | 'C' | 'D' | 'E' | 'F' // make ANTLRworks display separate branches
	;
...
CHARACTER :
	...
	' ' | '!' | '#' | '$' | '%' | '&' | '(' | ')' | '*' | '+' |
	',' | '-' | '.' | ':' | ';' | '<' | '=' | '>' | '?' | '@' |
	'[' | '\\' | ']' | '^' | '_' | '`' | '{' | '|' | '}' | '~' // make ANTLRworks display separate branches
	;
